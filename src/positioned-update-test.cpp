...
void
printCurrentRowPositionedUpdate(HSTMT hstmt)
{
	char		buf[40];
	int			col;
	SQLLEN		ind;
...
TEST_CASE("positioned-update-test", "[odbc]") {
	int			rc;
	...
	rc = SQLExecDirect(hstmt, (SQLCHAR *) "INSERT INTO pos_update_test SELECT g, g FROM generate_series(1, 10) g(g)", SQL_NTS);
	CHECK_STMT_RESULT(rc, "SQLExecDirect failed", hstmt);
	...
	for (i = 0; i < 5; i++)
	{
		...
		if (rc == SQL_SUCCESS)
		{
			...
			printCurrentRowPositionedUpdate(hstmt);
		}
		else
		{
