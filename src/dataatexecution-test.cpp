...
TEST_CASE("dataatexecution-test", "[odbc]") {
	...
	while ((rc = SQLParamData(hstmt, &paramid)) == SQL_NEED_DATA)
	{
	  ...
	  {
		  rc = SQLPutData(hstmt, (SQLPOINTER) "foo", 3);
		  CHECK_STMT_RESULT(rc, "SQLPutData failed", hstmt);
		  /* append more data */
		  rc = SQLPutData(hstmt, (SQLPOINTER) "bar", 3);
		  CHECK_STMT_RESULT(rc, "SQLPutData failed", hstmt);
	  }
	  else
	  {
	...
	{
		if (nprocessed == 1)
			rc = SQLPutData(hstmt, (SQLPOINTER) "foo", strlen("foo"));
		else if (nprocessed == 2)
			rc = SQLPutData(hstmt, (SQLPOINTER) "barf", strlen("barf"));
		else
		{
