...
#define SQL_ATTR_PGOPT_BATCHSIZE 50
...
static SQLRETURN	BatchExecute(HDBC conn, int batch_size)
{
	...
	rc = SQLExecDirect(hstmt, (SQLCHAR*)"INSERT INTO test_batch VALUES (?, ?)"
	...
	**/
	strncpy((char *) &strs[BATCHCNT - 3], "4-long", sizeof(strs[0]));
	rc = SQLExecDirect(hstmt, (SQLCHAR*)"INSERT INTO test_batch VALUES (?, ?)"
		" ON CONFLICT (id) DO UPDATE SET dt=EXCLUDED.dt"
		, SQL_NTS);
	b_result(rc, hstmt, BATCHCNT, status);
...
TEST_CASE("params-batch-exec-test", "[odbc]") {
	SQLRETURN rc;
	...
	rc = SQLExecDirect(hstmt, (SQLCHAR*)"drop table if exists test_batch", SQL_NTS);
